...
def runner(event_stopper):
    try:
        ...
        logger.info(f'exponential moving average alfa={alfa}')
        logger.info(f'stop motors after {after_how_many_cycles_to_stop} cycles of inactivity')
        logger.info(f'control loop running at {loop_frequency}Hz')
        ...
        while not event_stopper.is_set():
            ...
            if not tracked_index:
                ...
                logger.info(f'change tracked index to {tracked_index}')
            ...
            logger.debug(f'motor speed left={left_motor_speed} right={right_motor_speed}')
