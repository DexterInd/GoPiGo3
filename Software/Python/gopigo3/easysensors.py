...
def _ifMutexAcquire(mutex_enabled=False):
    ...
    """
    if mutex_enabled or mutex.overall_mutex():
        mutex.acquire()
...
class Sensor:
    ...
    def __str__(self):
        ...
        """
        return (f"{self.descriptor} on port {self.get_port()} \npinmode {self.get_pin_mode()}\nportID {self.portID}")
    ...
    def set_pin(self, pin):
        ...
        debug(f"setting pin to {self.pin}")
...
class DigitalSensor(Sensor):
    ...
    def read(self):
        ...
        except gopigo3.GoPiGoValueError:
            try:
                self.value = self.gpg.get_grove_state(self.get_pin())
            except Exception:
                return -1
...
class AnalogSensor(Sensor):
    ...
    def read(self):
        ...
        :rtype: int
        :raises gopigo3.GoPiGoValueError: If an invalid value was read.
        :raises Exception: If any other errors happens.

        """
        try:
            self.value = self.gpg.get_grove_analog(self.get_pin())
        except gopigo3.GoPiGoValueError as e:
            try:
                self.value = self.gpg.get_grove_analog(self.get_pin())
            ...
            print(f"Analog Read fail-all: {e}")
    ...
    def write_freq(self, freq):
        ...
        return_value = self.gpg.set_grove_pwm_frequency(
                self.get_port_ID(),
                self.freq)
        debug(f"Analog Write on {self.get_port_ID()} at {self.freq}")
        return return_value
##########################
...
class LightSensor(AnalogSensor):
    ...
    .. seealso::
         ...
         For more sensors, please see our Modular Robotics `shop`_.
...
class UltraSonicSensor(AnalogSensor):
    ...
    def read_mm(self):
        ...
        :rtype: int
        :raises gopigo3.GoPiGoValueError: If trying to read an invalid value.
