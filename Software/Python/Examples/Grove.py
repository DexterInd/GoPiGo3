#!/usr/bin/env python
#
# https://www.dexterindustries.com/GoPiGo/
# https://github.com/DexterInd/GoPiGo3
#
# Copyright (c) 2017 Dexter Industries
# Released under the MIT license (http://choosealicense.com/licenses/mit/).
# For more information see https://github.com/DexterInd/GoPiGo3/blob/master/LICENSE.md
#
# This code is an example for controlling the GoPiGo3 Motors
#
# Results:  When you run this program, the GoPiGo3 Motors will rotate back and forth.

from __future__ import print_function # use python 3 syntax but make it compatible with python 2
from __future__ import division       #                           ''

import time     # import the time library for the sleep function
import gopigo3 # import the GoPiGo3 drivers

GPG = gopigo3.GoPiGo3() # Create an instance of the GoPiGo3 class. GPG will be the GoPiGo3 object.

try:
    GPG.set_grove_type(GPG.GROVE_1, GPG.GROVE_TYPE.CUSTOM)
    GPG.set_grove_type(GPG.GROVE_2, GPG.GROVE_TYPE.CUSTOM)
    
    GPG.set_grove_mode(GPG.GROVE_1_1, GPG.GROVE_OUTPUT_PWM)
    GPG.set_grove_mode(GPG.GROVE_2, GPG.GROVE_INPUT_ANALOG)
    
    duty = 10
    #freq = 0
    while(True):
        print("Battery: %6.3fv   5v: %5.3fv   State: %d   Voltage: %4.2fv   ADC: %4d" % (GPG.get_voltage_battery(), GPG.get_voltage_5v(), GPG.get_grove_state(GPG.GROVE_2_2), GPG.get_grove_voltage(GPG.GROVE_2_2), GPG.get_grove_analog(GPG.GROVE_2_2)))
        #print("State: %d" % (GPG.get_grove_state(GPG.GROVE_1_2)[0]))
        #GPG.set_grove_state(GPG.GROVE_2_2, state)
        #state = not state
        duty = duty + 1 #GPG.get_grove_analog(GPG.GROVE_1_2)[0] / 40.95;
        if duty > 90:
            duty = 10
        GPG.set_grove_pwm_duty(GPG.GROVE_1_1, duty)
        
        #freq = freq + 10
        #if freq > 48000:
        #    freq = 0
        GPG.set_grove_pwm_frequency(GPG.GROVE_1, GPG.get_grove_analog(GPG.GROVE_2_2) * 10)
        time.sleep(0.1)

except KeyboardInterrupt: # except the program gets interrupted by Ctrl+C on the keyboard.
    GPG.reset_all()        # Unconfigure the sensors, disable the motors, and restore the LED to the control of the GoPiGo3 firmware.
