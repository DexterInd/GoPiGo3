...
try:
    while True:
        for i in range(11):                                # count from 0-10
            GPG.set_led(GPG.LED_1, i, i, i)                # set the LED brightness (0 to 255)
            GPG.set_led(GPG.LED_2, 10 - i, 10 - i, 10 - i) # set the LED brightness (255 to 0)
            GPG.set_led(GPG.LED_LEFT, (i * 25))           # set the LED brightness (0 to 255)
            GPG.set_led(GPG.LED_RIGHT, ((10 - i) * 25))     # set the LED brightness (255 to 0)
        ...
        for i in range(11):                                # count from 0-10
            GPG.set_led(GPG.LED_1, 10 - i, 10 - i, 10 - i) # set the LED brightness (255 to 0)
            GPG.set_led(GPG.LED_2, i, i, i)                # set the LED brightness (0 to 255)
            GPG.set_led(GPG.LED_LEFT, ((10 - i) * 25))    # set the LED brightness (0 to 255)
            GPG.set_led(GPG.LED_RIGHT, (i * 25))            # set the LED brightness (255 to 0)
            time.sleep(0.02)                               # delay for 0.02 seconds (20ms) to reduce the Raspberry Pi CPU load and give time to see the LED pulsing.
